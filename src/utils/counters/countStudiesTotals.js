export const countStudiesTotals = (studyList, studiesData, subjectsData) => {
  // Debug logging
  console.log('Function called with:', {
    studyList,
    subjectsDataLength: subjectsData?.length,
    firstSubject: subjectsData?.[0],
    studiesDataLength: studiesData?.length,
  });
  ...
  if (!studyList || !subjectsData || !studiesData) {
    console.warn('countStudiesTotals: Null or undefined inputs', {
      studyList,
      subjectsData,
      studiesData,
    });
  ...
  if (
    ...
    console.warn('countStudiesTotals: Inputs are not arrays', {
  ...
  const relevantSubjects = subjectsData.filter((subject) => {
    console.log('Checking subject:', {
      subjectStudy: subject?.study,
      isValid: validStudies.has(subject?.study),
    });
    return validStudies.has(subject?.study);
  });
